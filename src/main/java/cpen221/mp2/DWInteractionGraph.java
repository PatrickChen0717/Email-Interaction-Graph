...
public class DWInteractionGraph {
        ...
        try {
            ...
            for (String fileLine = reader.readLine();
                ...
                if(Integer.parseInt(String.valueOf(line[2]))<=timeFilter[1]&&Integer.parseInt(String.valueOf(line[2]))>=timeFilter[0]){
                    ...
                    if(!users.contains(sender) &&((userFilter.contains(receiver) || userFilter.contains(sender))||userFilter.size()==0)){
                        users.add(sender);
                        if(!users.contains(receiver)){
                            users.add(receiver);
                        }
                    }
                    if(!users.contains(receiver) &&((userFilter.contains(receiver) || userFilter.contains(sender))||userFilter.size()==0)){
                        users.add(receiver);
    ...
    public int getEmailCount(int sender, int receiver) {
        if(!allUsers.contains(sender) || !allUsers.contains(receiver)){
            return 0;
        }
    ...
    public int[] ReportOnUser(int userID) {
        ...
        return new int[]{sendCount,receiveCount,uniqueUsers.size()};
    }
   ...
   ArrayList<Integer> path=new ArrayList<>();
    ...
    public void task4helper(int user1, int time,ArrayList array, int hours) {
        for(int i=0;i<allUsers.size();i++){
            if(interactionMap[allUsers.indexOf(user1)][i].size()>0){
                for(int timechecker=0;timechecker<interactionMap[allUsers.indexOf(user1)][i].size();timechecker++){
                    ...
                    if((int)interactionMap[allUsers.indexOf(user1)][i].get(timechecker)>time&&(int)interactionMap[allUsers.indexOf(user1)][i].get(timechecker)<=hours){
                        ...
                        if(!array.contains(allUsers.get(i))){
    ...
    private void BFS_helper(int target, ArrayList<Integer> masterList, Set<Integer> revisitList){
        ...
        for(int i=0; i<masterList.size(); i++){
            ...
            for (Integer integer : temp) {
                if (!masterList.contains(integer)) {
                    masterList.add(integer);
                    if (integer == target) {
                        return;
                    }
                }
                if (!nextList.contains(integer)) {
